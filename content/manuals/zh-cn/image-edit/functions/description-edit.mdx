...
1. **指令清晰**：指令描述越清晰，修改效果越准确
2. **对象识别**：AI会自动识别目标对象，但复杂场景可能需要更精确的描述
3. **保持一致性**：修改后的内容会与周围环境保持视觉协调
4. **避免冲突**：避免在同一个指令中要求相互冲突的修改
...
1. **指令明确**：使用清晰、具体的语言描述修改需求
2. **目标明确**：明确指出要修改的对象和特征
3. **保持协调**：确保修改后的内容与整体环境协调
4. **避免过度**：避免要求过多或相互冲突的修改
